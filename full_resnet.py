...
def train_and_evaluate(input_dim, hidden_dims, learning_rate, batch_size):
    model = ResNet(input_dim, hidden_dims).to(device)
...
# Hyperparameter grid
hidden_dims_grid = [[256, 128, 64, 32], [512, 256, 128, 64], [128, 64, 32, 16]]
...
for hidden_dims, learning_rate, batch_size in tqdm(itertools.product(hidden_dims_grid, learning_rate_grid, batch_size_grid), desc="Hyperparameter Tuning", total=len(hidden_dims_grid) * len(learning_rate_grid) * len(batch_size_grid)):
    val_loss = train_and_evaluate(input_dim, hidden_dims, learning_rate, batch_size)
    if val_loss < best_val_loss:
        best_val_loss = val_loss
        best_params = (hidden_dims, learning_rate, batch_size)

print(f'Best params: {best_params}')
print(f'Best validation loss: {best_val_loss}')

# Train the final model with the best hyperparameters
best_hidden_dims, best_learning_rate, best_batch_size = best_params
model = ResNet(input_dim, best_hidden_dims).to(device)
